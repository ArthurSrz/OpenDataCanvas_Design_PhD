...
La deuxième étape consiste à sélectionner, par élément fonctionnel, les **fonctions** qui seront protoypées. Pour effectuer ce choix, nous utiliserons les critères du pattern *Easy solution first* (Vaishnavi et Kuelcher, 2015, p.131). Ce pattern vise à toujours privilégier les fonctions faciles à mettre en oeuvre en période de prototyage. Pour cela, il convient d'évaluer les fonctions en suivant le processus d'évaluation suivante : 

1. Si, pour une fonction, il existe une solution de prototypage facile à mettre en oeuvre et à évaluer, alors le chercheur-concepteur décide de l'utiliser et de l'évaluer 
2. Si cette solution fonctionne et génère l'effet souhaité, alors le chercheur-concepteur peut décider d'intégrer définitivement cette solution. 
3. Si la solution ne fonctionne pas ou aboutit à un résultat qui n'est pas raisonnable, alors le chercheur-concepteur peut justifier du choix de s'orienter vers une solution plus complexe. 


Autrement dit, nous priviligierons les fonctions qui peuvent être facilement mise en place par des outils logiciels connus et ne nécessitant pas de connaissances techniques. Cette façon de sélectionner les fonctions a notamment été choisi par Berners-Lee and Cailliau (1990) quand ils ont présenté pour la première fois le concept de *World Wide Web*. Internel était dépeint comme une combinaison de 2 éléments fonctionnels (un navigateur et un serveur) avec des fonctions simples : le navigateur a pour fonction de lire des liens hypertextextes qui correspondent à des noeuds d'un réseau et de garder un historique des liens lus ; le serveur, quant à lui, stocke tous les réseaux de noeuds et négocie avec le navigateur le format sous lequel les liens peuvent être lus. 
